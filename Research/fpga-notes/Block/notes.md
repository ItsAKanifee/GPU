...
- **8/24/26:** A bit of a late follow up, but I want to move path branching and memory retrieval into their own respective folders b/c these details are fairly complicated and require experimentation specific to those. What I have been able to achieve though is a gpu warp that can instructions into its instruction cache and run a simple program.
  - Short rundown of what is in the modules:
    ...
    - Instruction Cache: Cache to store the instructions for fast retrieval
    - Device Core: Module that holds and connects the Block, PC, and Instruction Cache

  - Futhermore:
    - There is a module called txt_reader, and what this does is transform symmbols on a txt file to hexadecimal, allowing for experimentation of different programs without having to recode a testbench, meaning vcs only needs to be ran once
      - Slight caveat: This can only interpret hexadecimal code at the moment, so any code that someone would want to run, they'd have to translate into hexadecimal
