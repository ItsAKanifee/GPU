module int_path#(parameter WIDTH = 32) ( // Basic datapath for one lane of GPU
    ...
    Mux #(.WIDTH(WIDTH)) A_mux(.I0(imm), .I1(out_A), .I2(LSU_rd), .I3('0), .sel(A_sel), .Iout(var_A)),
                        B_mux(.I0(imm), .I1(out_B), .I2(LSU_rd), .I3('0), .sel(B_sel), .Iout(var_B));
